    ...
    public function store(Request $request)
    {
        ...
        if (!empty($acceptData)) {
            ...
            foreach ($acceptData as $id) {
                ...
                DB::table('tbl_port_update')
                    ->where('serial_no', $id)
                    ->update(['approve_status' => 'Approved', 'approve_by' => $request->name, 'approve_date' => now(), 'entry_by' => auth()->user()->name]);
        ...
        if (!empty($rejectData)) {
            ...
            foreach ($rejectData as $id) {
                ...
                DB::table('tbl_port_update')
                    ->where('serial_no', $id)
                    ->update(['approve_status' => 'Rejected', 'approve_by' => $request->name, 'approve_date' => now(), 'entry_by' => auth()->user()->name]);
